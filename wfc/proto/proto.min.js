...
import PCOnlineType from "../model/pcOnlineType";
...
class WfcImpl {
	...
	setSendLogCommand(command) {
	    utsWfcClient.setSendLogCommand(command);
	}
	...
	clearFriendRequest(incomming, beforeTime) {
		utsWfcClient.clearFriendRequest(incomming?1:0, beforeTime);
	}
	
	deleteFriendRequest(userId, incomming) {
		clientModule.deleteFriendRequest(userId, incomming?1:0);
	}
    ...
    getConversationInfo(conversation) {
        ...
        if(!convStr){
            let convInfo = new ConversationInfo()
    ...
    markConversationAsUnread(conversation, syncToOtherClient) {
        ...
        if (result && syncToOtherClient) {
            let conversationInfo = self.getConversationInfo(conversation);
            self.eventEmitter.emit(EventType.ConversationInfoUpdate, conversationInfo);
            let content = new MarkUnreadMessageContent(result)
    ...
    getMessageCountByDay(conversation, contentTypes, startTime, endTime) {
        return utsWfcClient.getMessageCountByDay(JSON.stringify(conversation), contentTypes, startTime, endTime, startTime);
    }
    ...
    uploadMedia(fileName, data, mediaType, successCB, failCB, progressCB) {
    ...
    uploadMediaFile(fileName, mediaType, successCB, failCB, progressCB) {
        utsWfcClient.uploadMediaFile(fileName, mediaType,
            (remoteUrl) => {
                if (successCB) {
                    successCB(remoteUrl);
                }
    ...
    isAddFriendNeedVerify() {
        const setting = utsWfcClient.getUserSetting(UserSettingScope.AddFriend_NoVerify, '');
        return '1' !== setting;
    }

    setAddFriendNeedVerify(enable, successCB, failCB) {
        utsWfcClient.setUserSetting(UserSettingScope.AddFriend_NoVerify, '', enable ? '0' : '1', () => {
            successCB && successCB();
        }, err => failCB && failCB(err));
    }

    muteNotificationWhenPcOnline(isMute, successCB, failCB) {
        utsWfcClient.setUserSetting(UserSettingScope.MuteWhenPCOnline, '', isMute ? '0' : '1', () => {
            successCB && successCB();
        }, err => failCB && failCB(err));
    }
    ...
    kickoffPCClient(clientId, successCB, failCB) {
        utsWfcClient.kickoffPCClient(clientId, successCB, failCB);
    }

    saveToFile(name, data) {
        return utsWfcClient.saveToFile(name, data)
    }
    ...
    chooseFile(type, successCB, failCB) {
        utsWfcClient.chooseFile(type, (res) => {
            successCB && successCB(JSON.parse(res));
        }, (err) => {
            failCB && failCB(err);
        });
    }
